...
class TimeCost:
    ...
    def output_sdk_header(format_str, only_brief):
        delay_server_api_cost = "delay(server_api_cost){format_str}".format(
            format_str=format_str
        )
        delay_server_req_cost = "delay(server_req_cost){format_str}".format(
            format_str=format_str
        )
        sdk_api_cost = "sdk_api_cost{format_str}".format(format_str=format_str)

        sdk_test_start_time = "sdk_test_start_time{format_str}".format(
            format_str=format_str
        )
        ...
        if only_brief:
            print(delay_server_api_cost, delay_server_req_cost, sdk_api_cost)
        else:
            print(
    ...
    def output_sdk_cost(dict_data, format_str, only_brief):
        ...
        sdk_api_cost_desc = "{sdk_api_cost}{format_str}".format(
            sdk_api_cost=dict_data["sdk_api_cost"], format_str=format_str
        )

        sdk_func_name = dict_data.get("sdk_func_name", None)
        if sdk_func_name:
            sdk_func_name_desc = "{sdk_func_name}{format_str}".format(
                sdk_func_name=dict_data["sdk_func_name"], format_str=format_str
            )
        else:
            sdk_func_name_desc = ""
        ...
        if run_status:
            run_status_desc = "{run_status}{format_str}".format(
                run_status=dict_data["run_status"], format_str=format_str
            )
        else:
            run_status_desc = ""
        ...
        if only_brief:
            print(
                sdk_api_delay_desc, sdk_req_delay_desc, sdk_api_cost_desc,
            )
        else:
            print(
                sdk_api_delay_desc,
    ...
    def output_sort_cost(by_key_name, is_sorted=False):
        ...
        if is_sorted:
            output_list = sorted(
                time_cost_detail_list,
                key=lambda e: e.__getitem__(by_key_name),
                reverse=True,
            )
        else:
            output_list = time_cost_detail_list

        TimeCost.output_sdk_cost_list(
            data_list=output_list, format_str="\t", only_brief=False
        )
    ...
    def output_average_cost():
        ...
        if len(time_cost_detail_list):
            ...
            for key_name in sum_key_list:
                sum_final[key_name] = sum(
                    row[key_name] for row in time_cost_detail_list
                )
                average_final[key_name] = round(
                    sum_final[key_name] / average_count, ROUND_SIZE
                )
...
class RestfulTestCaseSeq:
    ...
    def test_generic(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = generic_client.get_exchange_timestamp()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = generic_client.get_exchange_currencies()
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = generic_client.get_exchange_symbols()
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = generic_client.get_reference_currencies()
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = generic_client.get_system_status()
        tc.run_status = (
            RunStatus.SUCCESS
            if result and result.get("page") and result.get("components")
            else RunStatus.FAILED
        )
        tc.add_record()

    def test_market(self):
        market_client = MarketClientPerformance(
            api_key=g_api_key, secret_key=g_secret_key
        )
        common_market_symbol = "btcusdt"

        # case get_candlestick
        tc = TimeCost(function_name=market_client.get_candlestick.__name__)
        result, tc.server_req_cost, tc.server_api_cost = market_client.get_candlestick(
            symbol=common_market_symbol, period=CandlestickInterval.MIN1, size=150
        )
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        ...
        result, tc.server_req_cost, tc.server_api_cost = market_client.get_pricedepth(
            symbol=common_market_symbol, depth_type=DepthStep.STEP0, depth_size=20
        )
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result.bids) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = market_client.get_market_detail(symbol=common_market_symbol)
        tc.run_status = RunStatus.SUCCESS if result and result.id else RunStatus.FAILED
        tc.add_record()

        # case get_market_trade
        tc = TimeCost(function_name=market_client.get_market_trade.__name__)
        result, tc.server_req_cost, tc.server_api_cost = market_client.get_market_trade(
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = market_client.get_history_trade(symbol=common_market_symbol)
        tc.run_status = (
            RunStatus.SUCCESS if result and result.count else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = market_client.get_market_detail_merged(symbol=common_market_symbol)
        tc.run_status = (
            RunStatus.SUCCESS
            if result and len(result.bid) and len(result.ask)
            else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = market_client.get_market_tickers()
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
    ...
    def test_wallet(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = wallet_client.get_account_deposit_address(currency="usdt")
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = wallet_client.get_account_withdraw_quota(currency="usdt")
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = wallet_client.get_sub_user_deposit_address(
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = wallet_client.get_sub_user_deposit_history(sub_uid=g_sub_uid)
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = wallet_client.get_deposit_withdraw(
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = wallet_client.post_create_withdraw(
        ...
        tc.run_status = (
            RunStatus.SUCCESS if record_id and record_id > 0 else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = wallet_client.post_cancel_withdraw(withdraw_id=record_id)
        tc.run_status = (
            RunStatus.SUCCESS if record_id and record_id > 0 else RunStatus.FAILED
        )
        tc.add_record()
    ...
    def test_cross_margin(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.post_cross_margin_transfer_in(
        ...
        tc = TimeCost(
            function_name=margin_client.get_cross_margin_account_balance.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.get_cross_margin_account_balance()
        tc.run_status = RunStatus.SUCCESS if result and result.id else RunStatus.FAILED
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.get_cross_margin_loan_info()
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        tc = TimeCost(
            function_name=margin_client.post_cross_margin_create_loan_orders.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.post_cross_margin_create_loan_orders(
            currency=loan_currency, amount=loan_amount
        )
        tc.run_status = (
            RunStatus.SUCCESS if result and int(result) > 0 else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.get_cross_margin_loan_orders(
        ...
        if cross_loan_accrual_order_list and len(cross_loan_accrual_order_list):
            for loan_order in cross_loan_accrual_order_list:
                tc = TimeCost(
                    function_name=margin_client.post_cross_margin_loan_order_repay.__name__
                )
                ...
                (
                    ...
                    tc.server_req_cost,
                    tc.server_api_cost,
                ) = margin_client.post_cross_margin_loan_order_repay(
        ...
        tc = TimeCost(
            function_name=margin_client.post_cross_margin_transfer_out.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.post_cross_margin_transfer_out(
    ...
    def test_margin(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.post_transfer_in_margin(
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.get_margin_loan_info(
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.get_margin_loan_orders(symbol=loan_symbol)
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.post_create_margin_order(
            symbol=loan_symbol, currency=loan_currency, amount=loan_amount
        )
        tc.run_status = (
            RunStatus.SUCCESS if result and int(result) > 0 else RunStatus.FAILED
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.get_margin_loan_orders(
        ...
        if cross_loan_accrual_order_list and len(cross_loan_accrual_order_list):
            for loan_order in cross_loan_accrual_order_list:
                tc = TimeCost(
                    function_name=margin_client.post_repay_margin_order.__name__
                )
                repay_amount = float(loan_order.loan_balance) + float(
                    loan_order.interest_balance
                )
                ...
                (
                    ...
                    tc.server_req_cost,
                    tc.server_api_cost,
                ) = margin_client.post_repay_margin_order(
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = margin_client.post_transfer_out_margin(
            symbol=loan_symbol,
    ...
    def test_trade_fee(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.get_transact_feerate(
            symbols="htusdt,btcusdt,eosusdt," + trade_symbol
        )
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
    ...
    def test_trade_order(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.order_id = trade_client.create_order(
            symbol=trade_symbol,
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.order_id = trade_client.get_order(order_id=order_id_tmp)
        tc.run_status = RunStatus.SUCCESS if result else RunStatus.FAILED
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.order_id = trade_client.get_order_by_client_order_id(
            client_order_id=client_order_id
        )
        tc.run_status = RunStatus.SUCCESS if result else RunStatus.FAILED
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.order_id = trade_client.get_open_orders(
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.order_id = trade_client.get_orders(
            symbol=trade_symbol,
            order_type=OrderType.BUY_LIMIT,
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.order_id = trade_client.cancel_order(
    ...
    def test_trade_create_cancel_orders(self):
        ...
        order_config_list = [buy_limit_eos_01, buy_limit_eos_02, buy_limit_eos_03]
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.batch_create_order(order_config_list=order_config_list)
    ...
    def test_trade_create_cancel_open_orders(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.cancel_open_orders(account_id=g_account_id)
        tc.run_status = RunStatus.SUCCESS if result else RunStatus.FAILED
        tc.add_record()
    ...
    def test_trade_match_result(self):
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = trade_client.get_match_results_by_order_id(order_id=created_order_id)
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
    ...
    def test_account(self):
        account_client = AccountClientPerformance(
            api_key=g_api_key, secret_key=g_secret_key, performance_test=True
        )
        # case get_accounts
        ...
        tc = TimeCost(
            function_name=account_client.get_account_balance_by_subuid.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.get_account_balance_by_subuid(sub_uid=g_sub_uid)
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        tc = TimeCost(
            function_name=account_client.get_account_by_type_and_symbol.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.get_account_by_type_and_symbol(
            account_type=AccountType.SPOT, symbol=None
        )
        account_id_spot = result.id if result else None
        tc.run_status = (
            RunStatus.SUCCESS
            if result and result.id and account_id_spot
            else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.get_account_history(account_id=account_id_spot)
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        (
            ...
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.get_account_ledger(account_id=account_id_spot)
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        tc = TimeCost(
            function_name=account_client.get_aggregated_subuser_balance.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.get_aggregated_subuser_balance()
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        tc.add_record()
        ...
        result, tc.server_req_cost, tc.server_api_cost = account_client.get_balance(
            account_id=account_id_spot
        )
        tc.run_status = (
            RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        )
        ...
        tc = TimeCost(
            function_name=account_client.post_sub_uid_management.__name__ + "_lock"
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.post_sub_uid_management(
            sub_uid=g_sub_uid, action=SubUidAction.LOCK
        )
        tc.run_status = (
            RunStatus.SUCCESS
            if result and (result.userState == SubUidState.LOCK)
            else RunStatus.FAILED
        )
        tc.add_record()
        ...
        # case post_sub_uid_management
        tc = TimeCost(
            function_name=account_client.post_sub_uid_management.__name__ + "_unlock"
        )
        (
            result,
            tc.server_req_cost,
            ...
            sub_uid=g_sub_uid, action=SubUidAction.UNLOCK
        )
        tc.run_status = (
            RunStatus.SUCCESS
            if result and (result.userState == SubUidState.NORMAL)
            else RunStatus.FAILED
        )
        tc.add_record()
        ...
        tc = TimeCost(
            function_name=account_client.transfer_between_futures_and_pro.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.transfer_between_futures_and_pro(
        ...
        tc = TimeCost(
            function_name=account_client.transfer_between_parent_and_subuser.__name__
        )
        (
            result,
            tc.server_req_cost,
            tc.server_api_cost,
        ) = account_client.transfer_between_parent_and_subuser(
            sub_uid=g_sub_uid,
            currency="usdt",
            amount=10,
