...
list_obj = margin_client.get_cross_margin_loan_orders(
    currency=currency, state=LoanOrderState.ACCRUAL
)
...
if list_obj and len(list_obj):
    for loan_order in list_obj:
        ...
        LogInfo.output(
            "repay loan order {id}, repay amount : {amount}".format(
                id=loan_order.id, amount=repay_amount
            )
        )
        result = margin_client.post_cross_margin_loan_order_repay(
            order_id=loan_order.id, amount=repay_amount
        )
        LogInfo.output(
            "step 3: repay loan order {id}, status : {status}, repay amount : {amount}".format(
                id=loan_order.id, status=result, amount=repay_amount
            )
        )
