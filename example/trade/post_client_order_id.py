...
client_order_id_test = (
    "client_" + client_order_id_header + "_order"
)  # unique id in 24hours
...
LogInfo.output(
    "======= create new order id : {order_id} with client id {client_id} =======".format(
        order_id=(order_id), client_id=client_order_id_test
    )
)

orderObj = trade_client.get_order(order_id=order_id)
LogInfo.output(
    "======= get order by order id : {order_id} =======".format(order_id=order_id)
)
...
LogInfo.output(
    "======= cancel order by client order id : {client_id} =======".format(
        client_id=client_order_id_test
    )
)
...
LogInfo.output(
    "======= get order by client order id : {client_id} after cancel =======".format(
        client_id=client_order_id_test
    )
)
orderObj.print_object()
